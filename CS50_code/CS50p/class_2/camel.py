...
is uppercase. For instance, whereas a variable for a user’s name might be called name,
a variable for a user’s first name might be called firstName, and a variable for a
user’s preferred first name (e.g., nickname) might be called preferredFirstName.
...
def to_snake(str_: str) -> str:
    outstr_ = ""
    for char_ in str_:
        if char_.isupper():
            outstr_ = outstr_ + ("_" + char_.lower())
        else:
            outstr_ = outstr_ + char_
            
    return outstr_


def main():
    camel_ = input("Please enter name of variable in camelCase: ")
    print(to_snake(camel_))
