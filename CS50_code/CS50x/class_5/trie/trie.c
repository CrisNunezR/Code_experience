...
int main(int argc, char *argv[])
{
    ...
    while (fscanf(infile, "%s", name) == 1)
    {
        ...
        //goes through the words/names on the file
        for (int i = 0, n = strlen(name); i < n; i++)
        {
            int index = tolower(name[i]) - 'a'; //by using the ascii format, defines the index of a letter
...
bool check(char* word)
{
    //create a cursos to go through the trie
    node *cursor = root;        // we define the cursor as the memory address of the root trie

    for (int i = 0, n = strlen(word); i < n; i++)
    {
        int index = tolower(word[i]) - 'a'; //defines index for current letter in word

        if (cursor->children[index] == NULL)
        {
            //node for letter doesn't exists in trie at location => word not found
            return false;
        }
        else
        {
            //node exists, reassign cursor to the next node/level
            cursor = cursor->children[index];

            //if last letter, check remaining letters
            //if not last letter just go forward checking next letter
            if (i == n - 1)
            {
                //if last letter on word, we need to check for an "is_word" attribute
